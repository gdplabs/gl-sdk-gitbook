...
**TXT Parser** is responsible for parsing the text structure within TXT documents. It maps loaded elements from the TXT Loader into structured elements.

This page provides guide to use TXT Parser in Document Processing Orchestrator (DPO).
...
You can use the following as a sample file: [loaded\_elements.json](https://assets.analytics.glair.ai/generative/txt/txtloader-output.json).
...
from gllm_docproc.parser.document import TXTParser
...
{% step %}
The parser will generate the following: [output JSON](https://assets.analytics.glair.ai/generative/txt/txtparser-output.json).
{% endstep %}
{% endstepper %}
